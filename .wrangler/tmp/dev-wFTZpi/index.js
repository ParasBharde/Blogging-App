...
// node_modules/@parasbarde/zod-validation/dist/index.js
var require_dist = __commonJS({
  "node_modules/@parasbarde/zod-validation/dist/index.js"(exports) {
    "use strict";
    init_checked_fetch();
    init_modules_watch_stub();
    var __importDefault = exports && exports.__importDefault || function(mod) {
...
// src/routes/user.ts
var import_zod_validation = __toESM(require_dist());
var app = new Hono2();
app.post("/signup", async (c) => {
  ...
  const { success } = import_zod_validation.signupSchema.safeParse(body);
  if (!success) {
    c.status(411);
...
app.post("/signin", async (c) => {
  ...
  const body = await c.req.json();
  const { success } = import_zod_validation.signinSchema.safeParse(body);
  if (!success) {
    c.status(411);
    return c.json("Input worngs");
  } else {
...
// src/routes/blogs.ts
var import_zod_validation2 = __toESM(require_dist());
...
blogApp.post("/create", authMiddleware, async (c) => {
  ...
  const body = await c.req.json();
  const { success } = import_zod_validation2.blogSchema.safeParse(body);
...
blogApp.put("/", authMiddleware, async (c) => {
  ...
  const body = await c.req.json();
  const { success } = import_zod_validation2.blogupdateSchema.safeParse(body);
  const userId = c.get("userId");
  if (!success) {
    c.status(411);
    return c.json("Input worngs");
